	...
	public static function convertFromOtf(array $cultivations): ?array {
		...
		foreach($cultivations as $cultivation) {
				...
				if(array_key_exists($seriesId, $seriesIndex)) {
					...
					$import[$index] = [
						'series' => [
							...
							'mode' => $line['mode'] ?: NULL,
							'use' => $line['use'] ?: NULL,
							'bed_length' => (int)round($line['bed_length']) ?: NULL,
							'block_area' => (int)round($line['block_area']) ?: NULL,
							'finished' => (($line['finished'] ?? 'false') === 'true'),
						],
						'cultivations' => []
					];
			...
			$import[$index]['cultivations'][] = [
				...
				'block_spacing_rows' => (int)round($line['block_spacing_rows']) ?: NULL,
				'block_spacing_plants' => (int)round($line['block_spacing_plants']) ?: NULL,
				'bed_density' => (float)$line['bed_density'] ? round((float)$line['bed_density'], 2) : NULL,
				'bed_rows' => (int)round($line['bed_rows']) ?: NULL,
				'bed_spacing_plants' => (int)round($line['bed_spacing_plants']) ?: NULL,
