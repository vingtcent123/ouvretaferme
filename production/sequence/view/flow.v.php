...
new AdaptativeView('create', function($data, PanelTemplate $t) {
	...
	return new \sequence\FlowUi()->create($data->e, $data->cAction);
...
new AdaptativeView('update', function($data, PanelTemplate $t) {
	...
	return new \sequence\FlowUi()->update($data->eSequence, $data->e, $data->cAction);

});

new AdaptativeView('incrementWeekCollection', function($data, PanelTemplate $t) {
	return new \sequence\FlowUi()->updateIncrementWeekCollection($data->c);
});

new JsonView('doUpdate', function($data, AjaxTemplate $t) {

	$t->js()->moveHistory(-1);
	$t->qs('#flow-wrapper')->outerHtml(new \sequence\FlowUi()->getTimeline($data->eSequence, $data->events, TRUE));
