	...
	public static function getFoodMessage(string $history, \Collection $cGrowing, array $values): string {
		...
		return match($history) {
			'harvesting' => ($values[0] > 0) ?
				s("Vous avez récolté {value}", ['value' => $values[0].'  '.GrowingUi::getVignette($cGrowing->first(), '1rem')]) :
				s("Vous avez consommé {value}", ['value' => $values[0].'  '.GrowingUi::getVignette($cGrowing->first(), '1rem')]),
