	...
	public static function convertFromOtf(array $cultivations): ?array {
		...
		foreach($cultivations as $cultivation) {
				...
				if(array_key_exists($seriesId, $seriesIndex)) {
					...
					$import[$index] = [
						'series' => [
							...
							'use' => $line['use'] ?: NULL,
							'bed_length' => (int)$line['bed_length'] ?: NULL,
							'block_area' => (int)$line['block_area'] ?: NULL,
							'finished' => (($line['finished'] ?? 'false') === 'true'),
						],
						'cultivations' => []
					];
			...
			$import[$index]['cultivations'][] = [
				...
				'block_spacing_rows' => (int)$line['block_spacing_rows'] ?: NULL,
				'block_spacing_plants' => (int)$line['block_spacing_plants'] ?: NULL,
				'bed_density' => (float)$line['bed_density'] ? round((float)$line['bed_density'], 2) : NULL,
				'bed_rows' => (int)$line['bed_rows'] ?: NULL,
				'bed_spacing_plants' => (int)$line['bed_spacing_plants'] ?: NULL,
	...
	public static function importCultivations(\farm\Farm $eFarm, array $list): bool {
		...
		foreach($list as ['series' => $series, 'cultivations' => $cultivations]) {
			...
			$input = [
				...
				'areaTarget' => $series['block_area'],
				'lengthTarget' => $series['bed_length'],
