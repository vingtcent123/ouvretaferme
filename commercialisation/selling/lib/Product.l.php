...
class ProductLib extends ProductCrud {
	...
	public static function create(Product $e): void {
		...
		if($e['farm']->hasAccounting()) {
			...
			if(isset($eConfiguration['profileAccount'][$e['profile']]['privateAccount'])) {
				$e['privateAccount'] = new \account\Account(['id' => $eConfiguration['profileAccount'][$e['profile']]['privateAccount']]);
			}

			if(isset($eConfiguration['profileAccount'][$e['profile']]['proAccount'])) {
			...
			Product::model()
				->select([
					'eItemTotal' => Item::model()
						->select([
							'product',
							'all' => new \Sql('ROUND(SUM(netPriceExcludingVat), 2)', 'float'),
							'year' => new \Sql('SUM(IF(EXTRACT(YEAR FROM deliveredAt) = '.date('Y').', netPriceExcludingVat, 0))', 'float'),
							'yearBefore' => new \Sql('SUM(IF(EXTRACT(YEAR FROM deliveredAt) = '.(date('Y') - 1).', netPriceExcludingVat, 0))', 'float'),
						])
