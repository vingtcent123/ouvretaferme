	...
	public static function getPreAccountingSales(\farm\Farm $eFarm, string $from, string $to): array {
		...
		foreach($cSale as $eSale) {
			...
			if($eSale->isMarketSale()) {
				...
				$eSaleParent = new Sale();
			}

			if($eSale['invoice']->empty()) {
				$document = $eSale['document'];
			} else {
				$document = $eSale['invoice']['name'];
			}

			if($eSale['cPayment']->notEmpty()) {
				$paymentMethod = $eSale['cPayment']->first()['method']['name'];
			} else {
				$paymentMethod = '';
			}
			...
			if($eSale->isComposition()) {
				...
				if($eSaleParent->exists()) {
					...
					$customerName = $eSaleParent['customer']['name'];
				} else {
					$customerId = $eSale['customer']['id'];
					$customerName = $eSale['customer']['name'];
				}
			...
			foreach($cItem as $eItem) {
				...
				$credit = $eItem['price'] < 0 ? 0 : abs($eItem['price']);

				if($cAccountSale->offsetExists($eAccount['id']) === FALSE) {

					$eAccount['debit'] = $debit;
					$eAccount['credit'] = $credit;

					$cAccountSale->offsetSet($eAccount['id'], $eAccount);

				} else {

					$cAccountSale[$eAccount['id']]['debit'] += $debit;
					$cAccountSale[$eAccount['id']]['credit'] += $credit;

				}

			}

			// On récupère direct les données de la vente
			if($cAccountSale->count() === 1) {

				// Vente
				$eAccount = $cAccountSale->first();
					$data[] = [
						$eAccount->exists() ? $eAccount['class'] : '',
						$eAccount->exists() ? $eAccount['description'] : '',
						$document,
						$date,
						$eSale['priceExcludingVat'] < 0 ? \util\TextUi::csvNumber(abs($eSale['priceExcludingVat'])) : 0,
						$eSale['priceExcludingVat'] > 0 ? \util\TextUi::csvNumber($eSale['priceExcludingVat']) : 0,
						$paymentMethod,
						$customerId,
						$customerName,
					];
				...
				// TVA
				if($eAccount['vatRate'] !== NULL or $eAccount['vatAccount']->notEmpty()) {

					$eAccountVat = $eAccount['vatAccount']->notEmpty() ? $cAccount->offsetGet($eAccount['vatAccount']['id']) : $eAccountVatDefault;

					$data[] = [
						$eAccountVat->exists() ? $eAccountVat['class'] : \account\AccountSetting::VAT_SELL_CLASS_ACCOUNT,
						$eAccountVat->exists() ? $eAccountVat['description'] : '',
						$document,
						$date,
						$eSale['vat'] < 0 ? \util\TextUi::csvNumber(abs($eSale['vat'])) : 0,
						$eSale['vat'] > 0 ? \util\TextUi::csvNumber($eSale['vat']) : 0,
						$paymentMethod,
						$customerId,
						$customerName,
					];
			...
			// Il faut ventiler par type de compte
			} else {

				foreach($cAccountSale as $eAccount) {

					if($eAccount['vatRate'] !== NULL or $eAccount['vatAccount']->notEmpty()) {
						$vatRate = $eAccount['vatRate'] ?? $eAccount['vatAccount']['vatRate'];
						$eAccountVat = $eAccount['vatAccount']->notEmpty() ? $cAccount->offsetGet($eAccount['vatAccount']['id']) : $eAccountVatDefault;
					} else {
						$vatRate = 0;
						$eAccountVat = new \account\Account();
					}
					...
					if($eAccount['debit'] > 0) {
						...
						$data[] = [
							...
							$document,
							$date,
							\util\TextUi::csvNumber($eAccount['debit'] / (1 + $vatRate / 100)),
							0,
							$paymentMethod,
							$customerId,
							$customerName,
						...
						if($eAccountVat->exists()) {
							$data[] = [
								$eAccountVat['class'],
								$eAccountVat['description'],
								$document,
								$date,
								\util\TextUi::csvNumber($eAccount['debit'] * $vatRate / 100),
								0,
								$paymentMethod,
								$customerId,
								$customerName,
					...
					if($eAccount['credit'] > 0) {
						...
						$data[] = [
							...
							$date,
							0,
							\util\TextUi::csvNumber($eAccount['credit'] / (1 + $vatRate / 100)),
							$paymentMethod,
							$customerId,
							$customerName,
						];
						...
						if($eAccountVat->exists()) {
							$data[] = [
								...
								$date,
								0,
								\util\TextUi::csvNumber($eAccount['credit'] * $vatRate / 100),
								$paymentMethod,
								$customerId,
								$customerName,
							];
