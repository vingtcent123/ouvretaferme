...
class SaleLib extends SaleCrud {
	...
	public static function fillForCreate(Sale $eSale): void {
		...
		if(get_exists('shopDate')) {
			...
			$eDate = \shop\DateLib::getById(GET('shopDate'), \shop\Date::getSelection() + [
		...
		if($eSale['customer']->empty()) {
			...
			if($eSale['shopDate']->notEmpty()) {
				$eSale['cProduct'] = \shop\ProductLib::exportAsSelling(\shop\ProductLib::getByDate($eSale['shopDate'], $eSale['customer'], withParents: FALSE));
