...
class AlertUi {
	...
	public static function getError(string $fqn, array $options): mixed {
		...
		return match($fqn) {
			...
			'Customer::fullElectronicAddress.check' => s("L'adresse de facturation électronique saisie ne respecte pas la norme. Elle doit comprendre un identifiant sur 4 chiffres et une adresse de facturation qui commence par les 9 premiers chiffres du SIRET."),
			...
			'Invoice::sales.month' => s("Vous ne pouvez pas mixer des ventes de mois différents au sein d'une même facture."),
			'Invoice::sales.profiles' => s("Vous ne pouvez pas mixer des achats et des ventes au sein d'une même facture."),
	...
	public static function getSuccess(string $fqn, array $options = []): ?string {
		...
		return match($fqn) {
			...
			'PaymentLink::created' => s("Le lien de paiement a bien été généré."),
