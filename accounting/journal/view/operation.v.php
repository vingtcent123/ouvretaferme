...
new AdaptativeView('createPayment', function($data, PanelTemplate $t) {
		...
		return new \journal\OperationUi()->createPayment($data->eCompany, $data->e, $data->cBankAccount);
...
new JsonView('getWaiting', function($data, AjaxTemplate $t) {
	...
	$t->qs('#waiting-operations-list')->innerHtml(new \journal\OperationUi()->listWaitingOperations($data->eCompany, $form, $data->cOperation));
