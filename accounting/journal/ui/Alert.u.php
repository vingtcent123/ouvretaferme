...
class AlertUi {
	...
	public static function getError(string $fqn, array $options): mixed {
		...
		return match($fqn) {
			...
			'Operation::date.locked' => s("Vous devez respecter la numérotation chronologique des dates de vos écritures. Compte tenu des écritures déjà validées, vous ne pouvez pas créer d'écriture antérieurement au {value}.", \util\DateUi::numeric(encode($options[0]))),
			...
			'Operation::vatRuleNoWithVatValue.inconsistency' => ($options[0] === '' ? s("L'une des écritures a de la TVA indiquée : choisissez une règle de TVA !") : s("L'une des écritures a de la TVA indiquée malgré une règle de TVA \"{value}\"", OperationUi::p('vatRule')->values[$options[0]])),
