					...
					$h .= '<tbody>';
						$h .= '<tr>';
							$h .= '<td rowspan="'.$rowspan.'" class="td-checkbox td-vertical-align-top">';
								$h .= '<input type="checkbox" name="batch[]" value="'.$eInvoice['id'].'" batch-type="market" oninput="Invoicing.changeSelection(this)" data-batch-amount-excluding="'.($eInvoice['priceExcludingVat'] ?? 0.0).'" data-batch-amount-including="'.($eInvoice['priceIncludingVat'] ?? 0.0).'" data-batch="'.implode(' ', $batch).'"/>';
							$h .= '</td>';
						...
						$h .= '<tr>';
							...
							$h .= '<td rowspan="'.$rowspan.'" class="td-vertical-align-top">';
								...
								$h .= '<div class="dropdown-list">';
									...
									$h .= '<a data-ajax="'.\company\CompanyUi::urlFarm($eFarm).'/invoicing/import:doIgnoreMarket" post-id="'.$eSale['id'].'" class="dropdown-item">'.s("Ignorer").'</a>';
								$h .= '</div>';
