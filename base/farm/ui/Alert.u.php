...
class AlertUi {
	...
	public static function getError(string $fqn, array $options): mixed {
		...
		return match($fqn) {
			...
			'Farm::notLegal' => '<p>'.s("Veuillez indiquer l'adresse de votre ferme pour continuer !").'</p><a href="/farm/farm:updateLegal?id='.$options['farm']['id'].'" class="btn '.($options['btn'] ?? 'btn-transparent').'">'.s("Compléter mes informations").'</a>',
			'Farm::notVatAccounting' => '<p>'.s("Veuillez paramétrer la TVA pour votre ferme pour continuer !").'</p><a href="/farm/configuration:updateVat?id='.$options['farm']['id'].'" class="btn '.($options['btn'] ?? 'btn-transparent').'">'.s("Compléter mes informations").'</a>',
	...
	public static function getErrorVatNumber(string $country): string {
		return match($country) {
			'FR' => s("Le numéro de TVA intracommunautaire pour une entité française doit commencer par FR et contenir 11 caractères"),
