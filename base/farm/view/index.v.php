...
new AdaptativeView('/ferme/{id}/precomptabilite', function($data, FarmTemplate $t) {
	...
	if($data->eFarm->hasAccounting() === FALSE) {
		...
		$t->mainTitle = '<h1>'.s("Précomptabilité de la ferme").'</h1>';
		...
		echo '<h3 class="mt-2">';
			...
			if($inFuture) {
				...
				echo '<a class="ml-1" data-url="'.\farm\FarmUi::urlSellingSalesAccounting($data->eFarm).'/product" data-toggle-item="collapsed" data-step="product" onclick="SellingAccounting.toggle(\'product\'); return true;">'.Asset::icon('chevron-right').'</a>';
				echo '<a class="ml-1 hide" data-toggle-item="expanded" data-step="product" onclick="SellingAccounting.toggle(\'product\')">'.Asset::icon('chevron-down').'</a>';
		...
		echo '<h3 class="mt-2">';
			...
			if($inFuture) {
				...
				echo '<a class="ml-1" data-url="'.\farm\FarmUi::urlSellingSalesAccounting($data->eFarm).'/item" data-toggle-item="collapsed" data-step="item" onclick="SellingAccounting.toggle(\'item\'); return true;">'.Asset::icon('chevron-right').'</a>';
				echo '<a class="ml-1 hide" data-toggle-item="expanded" data-step="item" onclick="SellingAccounting.toggle(\'item\')">'.Asset::icon('chevron-down').'</a>';
		...
		echo '<h3 class="mt-2">';
			...
			echo s("4. Vérification de l'état des ventes");
			...
			if($inFuture) {
				...
				echo '<span class="bg-warning tab-item-count ml-1">'.$nSale.'</span>';
				echo '<a class="ml-1" data-url="'.\farm\FarmUi::urlSellingSalesAccounting($data->eFarm).'/sale" data-toggle-item="collapsed" data-step="sale" onclick="SellingAccounting.toggle(\'sale\'); return true;">'.Asset::icon('chevron-right').'</a>';
				echo '<a class="ml-1 hide" data-toggle-item="expanded" data-step="sale" onclick="SellingAccounting.toggle(\'sale\')">'.Asset::icon('chevron-down').'</a>';

			}

		echo '</h3>';
		echo '<div data-step="sale"></div>';
...
new JsonView('/ferme/{id}/precomptabilite/{type}', function($data, AjaxTemplate $t) {
	...
	switch($data->type) {
		...
		case 'product':
			$t->qs('div[data-step="product"]')->innerHtml(new \selling\AccountingUi()->products($data->eFarm, $data->cProduct));
			break;
		...
		case 'sale':
			$t->qs('div[data-step="sale"]')->innerHtml(new \selling\AccountingUi()->salesTab($data->eFarm, $data->search, $data->saleCount));
			break;
...
new JsonView('/ferme/{id}/precomptabilite/sale/', function($data, AjaxTemplate $t) {

	$t->qs('div[data-step="sale-content"]')->innerHtml(new \selling\AccountingUi()->sales($data->cSale, $data->cPaymentMethod, $data->type));
	$t->qsa('a[data-sale-type]')->removeClass('selected');
	$t->qs('a[data-sale-type="'.$data->type.'"]')->addClass('selected');
